...
protected void tearDown() {
  ...
  // Algoritm 1:
  // H�mta alla klassvariabler (Identifiers/Path).
  // F�r varje metod M i klass K.
  // H�mta alla objektdeklereringar (Path/identifiers) t.ex. new File(varName)/File("/http/get/fetch.txt") och spara dessa i en en map<String, ArrayList<String>.


  // Algoritm 2:
  // G� igenom att metoderna,
  	// skapa ett objekt och spara bodyn i en lista ArrayList<MethodBody>
  	// Samla ihop alla klassvariabler (Identifiers/Path).
  	//
  // F�r varje metod, om metod == methodBody.name
  	// Kolla ifall method.body.contains(klassVariablerna):
  //

//  om metoden har en referens till en klassvariabel n�nstans i body, loopa isf genom resterande metoder.
  // Om b�de metoderna har en �verlappande klassvariabel, markera b�da som flaky.




	  // Algoritm 3:
	  // Samla ihop alla klassvariabler (Identifiers/Path) och l�gg till variabel container VC.

	  // F�r varje metod M
		  // Om en objectDeclarations sker, PATH/FILE, om variabel, l�gg den till MC [M: [metodvariabler]], om PATH/FILE: l�gg URLEN till VC.
		  // Om en methodCall, l�gg dessa argument och metodnamn i MC  M: [metodvariabler].
		  // Om ett objekt anropas, l�gg dess basvariabel och argument i MC  M: [metodvariabler].

	  // F�r varje metod M1 i MC
	  	// F�r varje metod M2 i MC
	  		// om M1.name != M2.name
	  		// F�r varje var i VC
	  			// Om M1.contains(var) && M2.contains(var)
	  				// Markera M1 & M2 som flaky (D� de delar en resurs)

	  // F�r varje metod M1 i MC
		// F�r varje metod M2 i MC
  			// om M1.vars.contain(M2.vars)
  				// markera b�da som flaky.



  				// OM M1.name.contains(VC) || M2.name.contains(VC)
  					// Markera de som �r sannasom flaky (d�



  	// Om ett en icke medlemsfunktionsanrop anropas och har en klassvariabel inparamter, l�gg dess bas in i flaky listan.

    // Om ett medlemsfunktionsanrop M1 har klassvariabel som inparameter som anropas i M, l�gg dessa in i flaky listan. ( Markera M och M1 som flaky.)
  ...
  // Observation Tv� fall:
  // (GO)   Fall 1: Man ser att att testdir �r inparameter till klassfunktionen delete() inuti tearDown vilket implicerar att delete() �r smelly.
  // (FAIL) Fall 2: I setup() kallas testdir av fs.delete() vilket implicerar de delar samma objekt, d�remot s� implicerar det inte att det �r specifikt samma resurs.


  //  Class variables
  // H�mta pathen och identifiern i new File(x), new Path(y) och spara i containern.

  // Objektdeklareringar
  //H�mta pathen i VariableDeclarationExpr f�r path/File, d� den anv�nds lokalt och spara i containern.


  // methodcallerexpr
  // om paramtetern �r en path/identifier som �r global finns i containern, markera metoden som kallas f�r smelly


  // om en globalt objekt anv�nder sig av path/identifier, l�gg till globala objektets instans i listan.
  //ELLER
  // Om ett objekt anv�nder sig av resuren likt fs.delete(testdir), l�gg till resursen fs i containern --- givet att den inte �r deklararad i scopet utan finns globally....Skapa en container med globals?


  // Om en metods body inneh�ller antingen identifiern/pathen/metodanrop, l�gg till metoden i containern, dvs, ifall en metod kallar p� den h�r metoden, g�r den smelly.
