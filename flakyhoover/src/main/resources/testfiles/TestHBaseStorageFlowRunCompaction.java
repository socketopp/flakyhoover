...
public class TestHBaseStorageFlowRunCompaction {
	...
	public void testWriteNonNumericData() throws Exception {
		...
		Table flowRunTable = conn.getTable(BaseTableRW.getTableName(hbaseConf, FlowRunTableRW.TABLE_NAME_CONF_NAME,
	...
	public void testWriteFlowRunCompaction() throws Exception {
		...
		try {
			...
			for (int i = start; i < start + count; i++) {
				...
				hbi.write(new TimelineCollectorContext(cluster, user, flow, flowVersion, runid, appName), te1,
						remoteUser);
		...
		TableName flowRunTable = BaseTableRW.getTableName(c1, FlowRunTableRW.TABLE_NAME_CONF_NAME,
				FlowRunTableRW.DEFAULT_TABLE_NAME);
	...
	private void checkFlowRunTable(String cluster, String user, String flow, long runid, Configuration c1,
		...
		Table table1 = conn.getTable(
				BaseTableRW.getTableName(c1, FlowRunTableRW.TABLE_NAME_CONF_NAME, FlowRunTableRW.DEFAULT_TABLE_NAME));
	...
	public void checkProcessSummationMoreCellsSumFinal2() throws IOException {
		...
		long cell2Ts = TimestampGenerator.getSupplementedTimestamp(System.currentTimeMillis(),
				"application_123746661110_11202");
		...
		Tag t = HBaseTimelineServerUtils.createTag(AggregationOperation.SUM_FINAL.getTagType(),
		...
		Cell c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cell1Ts,
				Bytes.toBytes(cellValue1), tagByteArray);
		currentColumnCells.add(c1);

		tags = new ArrayList<>();
		t = HBaseTimelineServerUtils.createTag(AggregationOperation.SUM_FINAL.getTagType(),
		...
		Cell c2 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cell2Ts,
				Bytes.toBytes(cellValue2), tagByteArray);
		currentColumnCells.add(c2);

		tags = new ArrayList<>();
		t = HBaseTimelineServerUtils.createTag(AggregationOperation.SUM.getTagType(),
				"application_191780000000001_8195");
		tags.add(t);
		tagByteArray = HBaseTimelineServerUtils.convertTagListToByteArray(tags);
		// create a cell with a VERY old timestamp but has attribute SUM
		Cell c3 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cell3Ts,
				Bytes.toBytes(cellValue3), tagByteArray);
		...
		tagByteArray = HBaseTimelineServerUtils.convertTagListToByteArray(tags);
		// create a cell with a VERY old timestamp but has attribute SUM
		Cell c4 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cell4Ts,
				Bytes.toBytes(cellValue4), tagByteArray);
		currentColumnCells.add(c4);

		List<Cell> cells = fs.processSummationMajorCompaction(currentColumnCells, new LongConverter(),
				currentTimestamp);
		assertNotNull(cells);
	...
	public void checkProcessSummationMoreCellsSumFinalMany() throws IOException {
		...
		for (int i = 0; i < count; i++) {
			...
			byte[] tagByteArray = HBaseTimelineServerUtils.convertTagListToByteArray(tags);
			// create a cell with a VERY old timestamp and attribute SUM_FINAL
			c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cellTsFinal,
					Bytes.toBytes(cellValueFinal), tagByteArray);
			currentColumnCells.add(c1);
			cellTsFinal++;
		}
		...
		for (int i = 0; i < count; i++) {
			...
			c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cellTsNotFinal,
					Bytes.toBytes(cellValueNotFinal), tagByteArray);
			currentColumnCells.add(c1);
			cellTsNotFinal++;
		}
	...
	public void checkProcessSummationMoreCellsSumFinalVariedTags() throws IOException {
		...
		long cellTsFinalStartNotExpire = TimestampGenerator.getSupplementedTimestamp(System.currentTimeMillis(),
		...
		for (int i = 0; i < countFinalNotExpire; i++) {
			...
			byte[] tagByteArray = HBaseTimelineServerUtils.convertTagListToByteArray(tags);
			// create a cell with a VERY old timestamp and attribute SUM_FINAL
			c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, cellTsFinalNotExpire,
					Bytes.toBytes(cellValueFinal), tagByteArray);
			currentColumnCells.add(c1);
	...
	public void testProcessSummationMoreCellsSumFinal() throws IOException {
		...
		// create a cell with a VERY old timestamp and attribute SUM_FINAL
		Cell c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, 120L, Bytes.toBytes(cellValue1),
				tagByteArray);
		currentColumnCells.add(c1);
		...
		// create a cell with a VERY old timestamp but has attribute SUM
		Cell c2 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, 130L, Bytes.toBytes(cellValue2),
				tagByteArray);
		currentColumnCells.add(c2);
		List<Cell> cells = fs.processSummationMajorCompaction(currentColumnCells, new LongConverter(),
				currentTimestamp);
	...
	public void testProcessSummationOneCellSumFinal() throws IOException {
		...
		Cell c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, 120L, Bytes.toBytes(1110L),
				tagByteArray);
		currentColumnCells.add(c1);
	...
	public void testProcessSummationOneCell() throws IOException {
		...
		List<Tag> tags = new ArrayList<>();
		Tag t = HBaseTimelineServerUtils.createTag(AggregationOperation.SUM.getTagType(),
				"application_123458888888_999888");
		tags.add(t);
		byte[] tagByteArray = HBaseTimelineServerUtils.convertTagListToByteArray(tags);
		...
		Cell c1 = HBaseTimelineServerUtils.createNewCell(aRowKey, aFamily, aQualifier, currentTimestamp,
				Bytes.toBytes(1110L), tagByteArray);
		currentColumnCells.add(c1);
		List<Cell> cells = fs.processSummationMajorCompaction(currentColumnCells, new LongConverter(),
				currentTimestamp);
		assertNotNull(cells);
