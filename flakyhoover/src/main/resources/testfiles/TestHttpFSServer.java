...
public class TestHttpFSServer extends HFSTestCase {
	...
	private Configuration createHttpFSConf(boolean addDelegationTokenAuthHandler, boolean sslEnabled) throws Exception {
		...
		conf.set("httpfs.admin.group",
				HadoopUsersConfTestHelper.getHadoopUserGroups(HadoopUsersConfTestHelper.getHadoopUsers()[0])[0]);
	...
	private void createHttpFSServer(boolean addDelegationTokenAuthHandler, boolean sslEnabled) throws Exception {
	...
	private void delegationTokenCommonTests(boolean sslEnabled) throws Exception {
		...
		Assert.assertEquals(sslEnabled ? WebHdfsConstants.SWEBHDFS_TOKEN_KIND : WebHdfsConstants.WEBHDFS_TOKEN_KIND,
				dToken.getKind());
	...
	public void instrumentation() throws Exception {
		...
		url = new URL(TestJettyHelper.getJettyURL(), MessageFormat
				.format("/webhdfs/v1?user.name={0}&op=instrumentation", HadoopUsersConfTestHelper.getHadoopUsers()[0]));
		conn = (HttpURLConnection) url.openConnection();
	...
	public void testMkdirs() throws Exception {
		...
		String user = HadoopUsersConfTestHelper.getHadoopUsers()[0];
		URL url = new URL(TestJettyHelper.getJettyURL(),
				MessageFormat.format("/webhdfs/v1/tmp/sub-tmp?user.name={0}&op=MKDIRS", user));
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("PUT");
		conn.connect();
		Assert.assertEquals(conn.getResponseCode(), HttpURLConnection.HTTP_OK);
	...
	private void createDirWithHttp(String dirname, String perms, String unmaskedPerms) throws Exception {
		...
		if (perms == null) {
			...
			pathOps = MessageFormat.format("/webhdfs/v1/{0}?user.name={1}&permission={2}&op=MKDIRS", dirname, user,
					perms);
		}
		if (unmaskedPerms != null) {
			pathOps = pathOps + "&unmaskedpermission=" + unmaskedPerms;
	...
	/**
	 * Talks to the http interface to get the json output of a *STATUS command on
	 * the given file.
	 *
	...
	/**
	 * General-purpose http PUT command to the httpfs server, which returns relted
	...
	private HttpURLConnection putCmdWithReturn(String filename, String command, String params) throws Exception {
		...
		String pathOps = MessageFormat.format("/webhdfs/v1/{0}?user.name={1}{2}{3}&op={4}", filename, user,
				(params == null) ? "" : "&", (params == null) ? "" : params, command);
		URL url = new URL(TestJettyHelper.getJettyURL(), pathOps);
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("PUT");
		conn.connect();
	...
	/**
	 * Given the JSON output from the GETFILESTATUS call, return the 'permission'
	 * value.
	 *
	...
	/**
	 * Given the JSON output from the GETACLSTATUS call, return the 'entries' value
	 * as a List<String>.
	 * 
	...
	/**
	 * Validate that files are created with 755 permissions when no 'permissions'
	 * attribute is specified, and when 'permissions' is specified, that value is
	 * honored.
	 */
	@Test
	@TestDir
	...
	public static String setXAttrParam(String name, byte[] value) throws IOException {
		return "xattr.name=" + name + "&xattr.value=" + XAttrCodec.encodeValue(value, XAttrCodec.HEX)
				+ "&encoding=hex&flag=create";
	}

	/**
	 * Validate the various ACL set/modify/remove calls. General strategy is to
	 * verify each of the following steps with GETFILESTATUS, LISTSTATUS, and
	 * GETACLSTATUS:
	 * <ol>
	...
	public void testFileAcls() throws Exception {
		...
		/*
		 * Now set an ACL on the file. (getfile|list)status have aclBit, and aclstatus
		 * has entries that looks familiar.
		 */
	...
	/**
	 * Test ACL operations on a directory, including default ACLs. General strategy
	 * is to use GETFILESTATUS and GETACLSTATUS to verify:
	 * <ol>
	...
	public void testCreateFileWithUnmaskedPermissions() throws Exception {
		...
		fs.mkdirs(new Path("/tmp"));
		AclEntry acl = new org.apache.hadoop.fs.permission.AclEntry.Builder().setType(AclEntryType.USER)
				.setScope(AclEntryScope.DEFAULT).setName("user2").setPermission(FsAction.READ_WRITE).build();
	...
	private HttpURLConnection snapshotTestPreconditions(String httpMethod, String snapOperation,
			String additionalParams) throws Exception {
		String user = HadoopUsersConfTestHelper.getHadoopUsers()[0];
		URL url = new URL(TestJettyHelper.getJettyURL(),
				MessageFormat.format("/webhdfs/v1/tmp/tmp-snap-test/subdir?user.name={0}&op=MKDIRS", user));
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("PUT");
		conn.connect();
		...
		DistributedFileSystem dfs = (DistributedFileSystem) FileSystem.get(snapshottablePath.toUri(),
				TestHdfsHelper.getHdfsConf());
		dfs.allowSnapshot(snapshottablePath);

		// Try to create snapshot passing snapshot name
		url = new URL(TestJettyHelper.getJettyURL(), MessageFormat.format(
				"/webhdfs/v1/tmp/tmp-snap-test?user.name={0}&op={1}&{2}", user, snapOperation, additionalParams));
		conn = (HttpURLConnection) url.openConnection();
	...
	public void testAllowSnapshot() throws Exception {
		...
		String user = HadoopUsersConfTestHelper.getHadoopUsers()[0];
		URL url = new URL(TestJettyHelper.getJettyURL(),
				MessageFormat.format("/webhdfs/v1{0}?user.name={1}&op=ALLOWSNAPSHOT", pathString, user));
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("PUT");
		conn.connect();
	...
	public void testDisallowSnapshot() throws Exception {
		...
		String user = HadoopUsersConfTestHelper.getHadoopUsers()[0];
		URL url = new URL(TestJettyHelper.getJettyURL(),
				MessageFormat.format("/webhdfs/v1{0}?user.name={1}&op=DISALLOWSNAPSHOT", pathString, user));
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
		conn.setRequestMethod("PUT");
		conn.connect();
		// Should return HTTP_OK
	...
	public void testCreateSnapshotNoSnapshotName() throws Exception {
		...
		Assert.assertTrue(Pattern
				.matches("(.+)(\\\"pathSuffix\\\":\\\"s)" + "(\\d{8})(-)(\\d{6})(\\.)(\\d{3})(\\\")(.+)", result));
	}
	...
	public void testRenameSnapshot() throws Exception {
		...
		Assert.assertEquals(conn.getResponseCode(), HttpURLConnection.HTTP_OK);
		conn = snapshotTestPreconditions("PUT", "RENAMESNAPSHOT",
				"oldsnapshotname=snap-to-rename" + "&snapshotname=snap-renamed");
		Assert.assertEquals(conn.getResponseCode(), HttpURLConnection.HTTP_OK);
	...
	private HttpURLConnection sendRequestToHttpFSServer(String path, String op, String additionalParams)
			throws Exception {
		String user = HadoopUsersConfTestHelper.getHadoopUsers()[0];
		URL url = new URL(TestJettyHelper.getJettyURL(),
				MessageFormat.format("/webhdfs/v1{0}?user.name={1}&op={2}&{3}", path, user, op, additionalParams));
		HttpURLConnection conn = (HttpURLConnection) url.openConnection();
	...
	private HttpURLConnection sendRequestGetSnapshotDiff(String path, String oldsnapshotname, String snapshotname)
			throws Exception {
		return sendRequestToHttpFSServer(path, "GETSNAPSHOTDIFF",
				MessageFormat.format("oldsnapshotname={0}&snapshotname={1}", oldsnapshotname, snapshotname));
	}
